...
var BgLayer = cc.Layer.extend({
	...
	init:function(){
		...
		nMapSize = this.map00.getContentSize();
		this.map00.setPosition(0, (wsize.height - nMapSize.height)/2);
		this.addChild(this.map00);
		this.mapWidth = this.map00.getContentSize().width;
		this.map01 = new cc.TMXTiledMap(res.map01_tmx);
		this.map01.setPosition(this.mapWidth, (wsize.height - nMapSize.height)/2);
...
      	this.addChild(this.spriteSheet);
	...
	update : function(dt){
		var antlayer = this.getParent().getChildByTag(TagOfLayer.animation);
		var nEyeX = antlayer.getEyeX();
		this.checkAndReload(nEyeX);
	},
